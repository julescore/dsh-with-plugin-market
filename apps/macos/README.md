...
`verify:macos` mounts the DMG in a clean location and checks the bundle identity, signature, runtime closure, plugin market registry, `dsh-web-ui` install/restart/uninstall path, both conflict choices, and a real `dshmarket@1.1.0` to `1.2.3` first-click update.
