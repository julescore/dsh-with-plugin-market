...
The application uses a .NET 8 WinForms shell with Microsoft Edge WebView2, bundles Node.js 24.19.0, pnpm 11.7.0, the production Harness runtime, and `dshmarket@1.2.3`, and generates an Inno Setup installer. Windows normally includes WebView2 Runtime; when it is absent the application presents an explicit installation requirement.
...
`verify:windows` silently installs into an isolated directory, runs the shell self-test, checks embedded Node and pnpm, starts the bundled Harness, reads the market registry, installs and removes the curated `dsh-web-ui` aggregate, and checks both plugin-market conflict compositions.
