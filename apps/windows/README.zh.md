...
`verify:windows` 会静默安装到隔离目录，运行外壳自检，检查内置 Node、pnpm 和安装包内恢复工具的行为，验证两个社区 preset 的行为，启动 Harness，通过 Host API 证明两个 preset 可发现且可挂载，读取市场目录、安装和卸载精选 `dsh-web-ui` 合集，并检查插件市场冲突的两种组合。
