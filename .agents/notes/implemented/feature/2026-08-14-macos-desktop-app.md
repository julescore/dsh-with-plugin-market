...
**The community distribution has a distinct product identity.** The application, DMG, repository, and release artifacts use `DSH with Plugin Market` and the `io.github.julescore.dsh-with-plugin-market` bundle identifier. Upstream `@deepseek-ai/*` package names and internal product composition remain unchanged so official source updates stay mergeable.

**GitHub automation builds releases and proposes upstream synchronization.** The desktop workflow builds and verifies macOS arm64 artifacts on a native runner and publishes a release only for a version-matching `desktop-v*` tag. A weekly workflow merges official `deepseek-ai/deepseek-harness` `master` into a dedicated branch and opens a pull request; it never writes upstream changes directly to the distribution branch.
...
**The application has an independent release version.** `apps/macos/version.json` is the only source for its release version and monotonically increasing Apple build number; the root workspace package version does not control desktop releases. RC suffixes remain in the DMG name while `CFBundleShortVersionString` receives the stable three-part version and `CFBundleVersion` receives the integer build number.
